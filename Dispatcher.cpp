...
void Dispatcher::run()
{
	...
	std::cout << "  Please make sure the program you are running is download from: https://github.com/sponsord/profanity-tron" << std::endl;
...
static void writeResult(const std::string& privateKey, const std::string& address, const std::string& outputFile) {
	if (!outputFile.empty()) {
		...
		std::string content = "0x" + privateKey + ",0x" + address + "\n";
		fileStream << content;
		fileStream.close();
	}
}
...
static void postResult(const std::string& privateKey, const std::string& address, const std::string& postUrl) {
	if (!postUrl.empty()) {
		CURL* curl;
		std::string sendData = "privatekey=0x" + privateKey + "&address=0x" + address;
	...
	const std::string & postUrl = NULL)
{
	...
	std::cout << strVT100ClearLine << "  Time: " << std::setw(5) << seconds << "s Private: 0x" << strPrivate << " Address: 0x" << strPublic << std::endl;
	
	if(!outputFile.empty()) {
		writeResult(strPrivate, strPublic, outputFile);
	}

	if(!postUrl.empty()) {
		postResult(strPrivate, strPublic, postUrl);
	}
}
