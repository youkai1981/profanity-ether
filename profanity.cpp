...
int main(int argc, char **argv)
{
	try
	{
		...
		if (prefixCount > 20)
		{
			std::cout << "error: the number of prefix matches cannot be greater than 20 :<" << std::endl;
			return 1;
		}
		...
		if (suffixCount > 20)
		{
			std::cout << "error: the number of suffix matches cannot be greater than 20 :<" << std::endl;
			return 1;
		}
		...
		else
		{
			...
			const char *szKernels[] = {kernel_keccak.c_str(), kernel_profanity.c_str()};
